...
window.GlobalLayers = (() => {
  ...
  function parseNdbcHistory(raw, field = "WVHT", maxRows = 72) {
    const colIdx = { WVHT: 8, WSPD: 6, ATMP: 12, WTMP: 13, PRES: 10 };
    const idx = colIdx[field] ?? 8;
  ...
  async function fetchCyclones(api) {
    const data = await api("nhc", "/CurrentStorms.json", {});
    const items = new Map();
    for (const s of data?.activeStorms || []) {
      const lat = s.latitudeNumeric ?? parseFloat(String(s.latitude).replace(/[NS]/i, ""));
      const lon = s.longitudeNumeric ?? parseFloat(String(s.longitude).replace(/[EW]/i, ""));
      if (!Number.isFinite(lat) || !Number.isFinite(lon)) continue;
      if (/W/i.test(String(s.longitude))) lon = -Math.abs(lon);
      if (/S/i.test(String(s.latitude))) lat = -Math.abs(lat);
      items.set(`cyclone-${s.id}`, {
        kind: "cyclone", id: `cyclone-${s.id}`,
        name: s.name || s.id, classification: s.classification || "",
        intensity: s.intensity, pressure: s.pressure,
        lat, lon, movementDir: s.movementDir, movementSpeed: s.movementSpeed,
        lastUpdate: s.lastUpdate, basin: s.binNumber || "", raw: s,
      });
    }
    return items;
  }
  ...
  async function fetchMetar(api, bounds) {
    ...
    const data = await api("aviation", "/api/data/metar", {
      format: "json", hours: 1,
      bbox: `${bounds.lonLeft},${bounds.latBottom},${bounds.lonRight},${bounds.latTop}`,
    });
    const items = new Map();
  ...
  async function fetchAirQuality(api) {
    ...
    const data = await api("openmeteo", "/v1/air-quality", {
  ...
  return {
    isElevatedVolcano, normQuake, normVolcano, parseNdbcHistory,
    parseRa, parseDec, zenithSubpoint, gmstDegrees,
    formatRaHms, formatDecDms,
    fetchEarthquakes, fetchVolcanoes, fetchCyclones, fetchTsunami, fetchVolcanicAsh,
    fetchFireballs, fetchBuoys, fetchMetar, fetchAirQuality, fetchAircraft,
